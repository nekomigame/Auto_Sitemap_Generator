...
- **アクセス制限（レートリミット）自動検知と待機機能**: `429 Too Many Requests` やページ内の制限キーワード（例: "アクセス制限"）を検知すると、すべての並列ワーカーを一時停止し、指定時間（`backoff_delay`）待機した後に自動で安全に再開します。
- **負荷軽減のためのジッター遅延**: リクエスト間に設定されたディレイ（`request_delay`）に対してランダムな揺らぎ（70%〜130%）を加えて待機することで、画一的なアクセスパターンを避けてサーバーへの負荷を軽減します。
- **安全な中断処理**: 実行中に `Ctrl+C` などで処理を中断しても、それまでに取得できたデータを用いてサイトマップを構築・保存してから安全に終了します。
- **日本語URLのデコード**: URLエンコードされた日本語の階層やファイル名などを自動でデコード（`unquote`）し、見やすく直感的なツリー構造で出力します。
